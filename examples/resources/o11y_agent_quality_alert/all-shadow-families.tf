...
resource "o11y_slo_burn_alert" "availability" {
  ...
  evaluation_settings_json    = jsonencode({ pending_for_secs = 120 })
  evaluation_interval_seconds = 60
  sample_guard_json           = jsonencode({ min_events = 100 })
  recipe_config_json = jsonencode({
...
resource "o11y_advanced_signal_alert" "latency" {
  ...
  evaluation_interval_seconds = 60
  sample_guard_json           = jsonencode({ min_events = 100 })
  recipe_config_json          = jsonencode({ metric = "latency_ms", operator = ">", threshold = 2000 })
  paused                      = false
  notify                      = false
}
