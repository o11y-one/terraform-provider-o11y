...
resource "o11y_alert_notification_policy" "live" {
  ...
  config_json = jsonencode({
    routes = [{
      route_key = %q
      destination_id = o11y_alert_destination.live.id
      matcher = {}
      priority = 10
      enabled = true
    }]
...
resource "o11y_alert_maintenance_window" "live" {
  ...
  scope_json = jsonencode({ service = %q })
...
resource "o11y_alert_silence" "live" {
  silence_key = %q
  matcher_json = jsonencode({ service = %q })
...
resource "o11y_agent_quality_alert" "live" {
  ...
  action_json = jsonencode({ runbook = "https://runbooks.example.test/terraform-live" })
