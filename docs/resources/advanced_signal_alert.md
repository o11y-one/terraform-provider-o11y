...
`AlertSampleGuardV1`. Every field is optional. The server stores both fields with each revision and returns them, but no evaluator reads them: the detector takes its minimum sample from `recipe_config_json`.
...
`AdvancedSignalConfigV1`. Detects symptoms: log errors, tool failures, provider fallbacks, and cache misses.
...
| Field | Type | Default | Description |
| --- | --- | --- | --- |
| `min_log_errors` | uint64 | `10` | Fewest log errors that count as a symptom, at least 1. |
| `max_log_error_rate` | double | `0.10` | Highest acceptable log error rate. |
| `min_tool_failures` | uint64 | `5` | Fewest tool failures that count as a symptom, at least 1. |
| `min_fallback_count` | uint64 | `5` | Fewest provider fallbacks that count as a symptom, at least 1. |
| `max_cache_miss_rate` | double | `0.90` | Highest acceptable cache miss rate. |
| `symptom_only_page_override` | bool | `false` | Let this symptom alert page on its own once notify is active. |
...
```shell
terraform import o11y_advanced_signal_alert.example <alert definition id>
```
...
- `recipe_config_json`: `min_log_errors`, `min_tool_failures`, `min_fallback_count`.
