...
`AlertSampleGuardV1`. Every field is optional. The server stores both fields with each revision and returns them, but no evaluator reads them: the detector takes its minimum sample from `recipe_config_json` (`min_success_count`).
...
`CostPerSuccessConfigV1`. Detects a rise in LLM spend per successful run, and optionally a budget running out.
...
| Field | Type | Default | Description |
| --- | --- | --- | --- |
| `window_seconds` | int64 | `3600` | Window spend and successes are summed over. |
| `min_success_count` | uint64 | `20` | Fewest successful runs a window needs, at least 1. |
| `max_cost_per_success_usd` | double | `0.10` | Highest acceptable spend per success, above 0. |
| `baseline_cost_per_success_usd` | double | `0.02` | Expected spend per success, above 0 and at most `max_cost_per_success_usd`. |
| `regression_multiplier` | double | `2` | How many times the baseline counts as a regression, at least 1. |
| `allow_estimated_cost` | bool | `false` | Count estimated cost when exact cost is missing. |
| `budget_remaining_usd` | double | none | Remaining budget, 0 or more. Omit it to skip the exhaustion check; it is the one field with no default. |
| `imminent_exhaustion_hours` | double | `24` | Alert when the budget runs out within this many hours, above 0. |
...
```shell
terraform import o11y_cost_per_success_alert.example <alert definition id>
```
...
- `recipe_config_json`: `window_seconds`, `min_success_count`.
